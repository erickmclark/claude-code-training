...
If Claude Code reports that the plugin is not found in any marketplace, run `/plugin marketplace update claude-plugins-official` to refresh it, or `/plugin marketplace add anthropics/claude-plugins-official` if you haven't added it before. Then retry the install.
...
**Step 6: Restart with channels enabled**
...
If your bot doesn't respond, make sure Claude Code is running with `--channels` from the previous step.
...
**Step 3: Restart with channels enabled**
...
**Step 1: Install the fakechat channel plugin**
...
**Step 2: Restart with the channel enabled**
