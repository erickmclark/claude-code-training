...
* **Windows x64**: For x64 processors — https://claude.ai/api/desktop/win32/x64/setup/latest/redirect
* **Windows ARM64**: For ARM processors — https://claude.ai/api/desktop/win32/arm64/setup/latest/redirect
...
* Windows x64: for x64 processors
* Windows ARM64: for ARM processors
...
Click the **Code** tab at the top center. If clicking Code prompts you to upgrade, you need to subscribe to a paid plan first. If it prompts you to sign in online, complete the sign-in and restart the app. If you see a 403 error, see authentication troubleshooting in the full Desktop guide.
...
Select a model from the dropdown next to the send button. See the models reference for a comparison of Opus, Sonnet, and Haiku. You cannot change the model after the session starts.
...
**Give Claude more context.** Type `@filename` in the prompt box to pull a specific file into the conversation, attach images and PDFs using the attachment button, or drag and drop files directly into the prompt. The more context Claude has, the better the results.
